...
object I18N {
    ...
    operator fun get(
        key: String,
        count: Number,
    ) = format(choose(bundle[key], count), count)
    ...
    operator fun get(
        ...
        count: Number,
        vararg args: Any,
    ) = format(choose(bundle[key], count), count, *args)
    ...
    private fun format(
        template: String,
        vararg args: Any,
    ) = MessageFormat.format(template.replace("'", "''"), *args)!!

    private fun choose(
        template: String,
        count: Number,
    ) = ChoiceFormat(template).format(count)!!
