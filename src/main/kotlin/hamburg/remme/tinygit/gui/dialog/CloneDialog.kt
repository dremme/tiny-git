...
class CloneDialog(
    window: Window,
) : Dialog<Unit>(window, I18N["dialog.clone.title"]) {
    ...
    init {
        ...
        content =
            grid(3) {
                ...
                +listOf(
                    Label("${I18N["dialog.clone.remote"]}:"),
                    url,
                    Label("${I18N["dialog.clone.location"]}:"),
                    location,
                    locationSet,
                    Label("${I18N["dialog.clone.userName"]}:"),
                    userName,
                    Label("${I18N["dialog.clone.userEmail"]}:"),
                    userEmail,
                    Label("${I18N["dialog.clone.proxy"]}:"),
                    proxy,
                )
            }
