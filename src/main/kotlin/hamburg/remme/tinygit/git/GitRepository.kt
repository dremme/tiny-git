...
private val notFound = "$FATAL_SEPARATOR.*repository.*not found".toRegex(IC)
private val notExist = "$FATAL_SEPARATOR.*repository.*does not exist".toRegex(IC)
...
fun gitClone(
    ...
    if (response.contains("$FATAL_SEPARATOR.*already exists".toRegex(setOf(IC, G)))) {
