...
fun List<KClass<*>>.createSingletonMap(): Map<KClass<*>, Any> {
    ...
    sortedByDependencies().forEach {
        ...
        val arguments = constructor.parameters.map { dependencyMap[it.type.jvmErasure] }
        dependencyMap += it to constructor.call(*arguments.toTypedArray())
    }
...
private fun List<KClass<*>>.sortedByDependencies() = sortedWith(Comparator { s1, s2 ->
    val param1 = s1.primaryConstructor!!.parameters
    val param2 = s2.primaryConstructor!!.parameters
    when {
    ...
    // Both classes depend on each other -> cyclic dependency
        param1.any { it.type.jvmErasure == s2 } && param2.any { it.type.jvmErasure == s1 }
        -> throw RuntimeException("Cyclic dependency between $s1 and $s2 detected.")
    // One class has the other one as dependency
        param2.any { it.type.jvmErasure == s1 } -> -1
        param1.any { it.type.jvmErasure == s2 } -> 1
